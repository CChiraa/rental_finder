class LandlordChatManager {
  static final List<Map<String, dynamic>> chats = [];

  static Map<String, dynamic> getOrCreateChat({
    ...
    final existingIndex = chats.indexWhere(
      (chat) =>
          chat['propertyId'] == propertyId && chat['tenantName'] == tenantName,
    );
  ...
  static void receiveTenantMessage(Map<String, dynamic> chat, String text) {
    final trimmed = text.trim();
    if (trimmed.isEmpty) return;
  ...
  static void receiveTenantImageMessage(
  ...
  static void _moveChatToTop(Map<String, dynamic> chat) {
    final int index = chats.indexWhere(
      (item) =>
          item['propertyId'] == chat['propertyId'] &&
          item['tenantName'] == chat['tenantName'],
    );
